...
CREATE TABLE IF NOT EXISTS invoices (id SERIAL PRIMARY KEY, invoice_number VARCHAR UNIQUE, client_name VARCHAR NOT NULL, client_email VARCHAR, status invoice_status NOT NULL DEFAULT 'draft', issue_date DATE NOT NULL, due_date DATE NOT NULL, subtotal FLOAT NOT NULL DEFAULT 0, tax_rate FLOAT NOT NULL DEFAULT 15, tax_amount FLOAT NOT NULL DEFAULT 0, total FLOAT NOT NULL DEFAULT 0, notes TEXT, invoice_type VARCHAR DEFAULT 'simplified', seller_vat VARCHAR, buyer_vat VARCHAR, buyer_address VARCHAR, supply_date DATE, zatca_qr TEXT, zatca_xml TEXT, zatca_hash VARCHAR, zatca_status VARCHAR DEFAULT 'pending', created_at TIMESTAMPTZ NOT NULL DEFAULT now());
